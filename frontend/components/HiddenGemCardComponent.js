class HiddenGemCardComponent extends HTMLElement {
  connectedCallback() {
    const name = this.getAttribute("name") || "Hidden Gem";
    ...
    const imageUrl =
      ...
      "https://images.pexels.com/photos/460376/pexels-photo-460376.jpeg?auto=compress&cs=tinysrgb&w=600";
    ...
    if (provinceLower.includes("palawan")) badgeColor = "bg-blue-500";
    else if (
    ...
    this.innerHTML = `
      <div class="relative z-10 card bg-black shadow-lg w-full h-full flex flex-col hover:scale-105 transition-transform cursor-pointer" data-place-id="${placeId}">
          <figure class="h-64 w-full overflow-hidden flex-shrink-0">
              <img src="${imageUrl}" alt="${name}" loading="lazy" class="w-full h-full object-cover"
              onerror="this.src='https://images.pexels.com/photos/460376/pexels-photo-460376.jpeg?auto=compress&cs=tinysrgb&w=600'" />
          </figure>
...
customElements.define("hidden-gem-card", HiddenGemCardComponent);
